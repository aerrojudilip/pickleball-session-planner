...
test("service worker caches the full shell and reloads offline", async ({ context, page }) => {
  ...
  const cacheState = await page.evaluate(async () => {
    ...
    const cache = await caches.open("pickleball-v9");
  ...
  expect(cacheState.names).toContain("pickleball-v9");
...
test("service worker registers when cloud startup finishes after window load", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(async () => (await caches.keys()).includes("pickleball-v9"))).toBe(true);
});
